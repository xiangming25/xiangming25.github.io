...
#### 1. transition如果下面有两个及以上的节点，需要给每个节点都设置一个key
...
#### 16. 如果在子组件中，只是想更新 props 当中的一个值，可以在 props 的属性上添加一个 .sync 属性，然后在子组件中使用 update:propName的yyaa
...
#### 22. 使用 mixins 时，同名钩子函数将合并成为一个数组，因此都将被调用。另外，`混入对象的钩子将在组件自身钩子之前调用`，值为对象的选项，例如 methods 将被合并为同一个对象。两个对象键名冲突时，取组件对象的键值对。
